...
from .modules import ART, OCVA, GlobalMultiViewFusion
from .gpu_preprocess import qwen_vl_preprocess_torch, resize_image_mask_torch
from .losses import CombinedLoss


class CrossViewerModel(nn.Module):
    """CrossViewer model for object-centric multi-view reasoning."""
    ...
    def __init__(self,
                 ...
                 no_mask_mode="sample_tokens"):
        super().__init__()

        print("=" * 80)
        print("Initializing CrossViewer Model...")
        print("=" * 80)
        ...
        self.no_mask_mode = (no_mask_mode or "sample_tokens").lower()
    ...
    def forward(self, pixel_values, image_grid_thw, masks=None, raw_images=None, questions=None, answers=None,
        ...
        if use_pixelrefer:
            ...
            if masks is not None:
                ...
                # No masks: treat each view as a single "object" built from whole-image features.
                object_tokens_flat = []
                object_valid_flat = []
                object_token_masks_flat = []
                for feat_map in feature_maps:
                    H, W, D = feat_map.shape
                    flat_tokens = feat_map.view(H * W, D)
                    if self.no_mask_mode == "global_pool":
                        pooled = flat_tokens.mean(dim=0, keepdim=True)  # [1, D]
                        tokens = pooled.unsqueeze(0)  # [1, 1, D]
                    elif self.no_mask_mode == "sample_tokens":
                        tokens = flat_tokens.unsqueeze(0)  # [1, H*W, D]
                        ...
                        raise ValueError(
                            f"Unsupported no_mask_mode={self.no_mask_mode!r}. "
                            "Expected 'sample_tokens' or 'global_pool'."
                        )
                    object_tokens_flat.append(tokens)  # [1, num_tokens, D] or [1, 1, D]
                    object_valid_flat.append(torch.ones(1, dtype=torch.bool, device=feat_map.device))
                    object_token_masks_flat.append(
                        torch.ones(tokens.shape[:2], dtype=torch.bool, device=feat_map.device)
                    )
        ...
        if questions is not None and region_refs is not None:
            ...
            for b_idx, sample_tokens in enumerate(object_tokens_per_sample):
                ...
                need_raw_region_features = False
                if refs:
                    for ref in refs:
                        if not isinstance(ref, (list, tuple)) or len(ref) < 2:
                            continue
                        try:
                            if int(ref[0]) > 1:
                                need_raw_region_features = True
                                break
                        except Exception:
                            continue

                if region_source == "fused" and not need_raw_region_features:
                    k_a = None
                    k_b = None
...
if __name__ == "__main__":
    ...
    print("Testing CrossViewerModel...")
    model = CrossViewerModel(
        vision_encoder_path="Qwen/Qwen3-VL-8B-Instruct",
        freeze_vision_encoder=True,
        freeze_llm=True
    )
